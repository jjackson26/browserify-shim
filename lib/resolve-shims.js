...
var resolve = module.exports = function resolveShims(file, messages, cb, shimConfig) {
  // find the package.json that defines browserify-shim config for this file
  mothership(file, function (pack) {
    return !! pack['browserify-shim'] || !! shimConfig
  }, function (err, res) {
    ...
    var packageDir = path.dirname(packFile);
      ...
      // if a config was passed in, use that
    if (!!shimConfig) {
      pack['browserify-shim'] = shimConfig;
    }
